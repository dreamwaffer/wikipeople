...
def getRawSparqlData(fromYear=None, toYear=None):
    """This method gets data about all the people from wikidata between certain years.
        ...
        Keyword arguments:
        fromYear -- beginning of the range (default 1850)
...
def getMetadataAndLinksForChunk(titlesDict, data):
    ...
    for page in pages:
        if 'original' in page:
            ...
            print(f'picture {page["title"]} does not exist')
        ...
        for image in person['images'].values():
            if not os.path.isfile(f'{constants.IMAGES_DIRECTORY}/{image["fileNameLocal"]}'):
                if 'url' in image:
                    success = False
                    counter = 0
                    while not success and counter < constants.IMAGE_DOWNLOAD_REPEATS:
                        counter += 1
                        ...
                        if response.ok:
                            ...
                            if isImageBroken(location):
                                success = False
                            else:
                                success = True
                        else:
                            print(
                                f'Image {image["fileNameWiki"]} which belongs to {person["wikidata"]} not found! REMOVING IT!')
                            del person['images'][image['fileNameWiki']]
                            success = True
                else:
                    print(f'Image {image["fileNameWiki"]} has no URL')
