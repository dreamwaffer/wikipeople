...
def reduceDatesForAll():
    data = merger.mergeAllData()
    print(data['Q1064677'])
    data = merger.mergeListOfValues(data)
    print(data['Q1064677'])
    splitData = merger.splitAllData(data)
    if not os.path.exists(f'{constants.DATA_DIRECTORY}/corrected'):
        os.makedirs(f'{constants.DATA_DIRECTORY}/corrected')
    for key, value in splitData.items():
        utils.saveData(value, f'{constants.DATA_DIRECTORY}/corrected/{key}.json')
