...
def train_model(model, criterion, optimizer, scheduler, num_epochs=25):
    ...
    for index, (phase, values) in enumerate(progress.items()):
        ...
        plt.title(f'Accuracy of model on {"training" if phase == "train" else "validation"} data throughout the training')
        plt.savefig(f'{resultDirectory}/acc{index}', dpi=300)
        ...
        for i, (inputs, labels) in enumerate(dataloaders['test']):
            ...
            if i < 10:
                ...
                out = torchvision.utils.make_grid(inputs.cpu().data,
                                                  out=torchvision.utils.make_grid(inputs.cpu().data))
                imsave(out, f'{resultDirectory}/test{i}.png', real=[class_names[x] for x in labels],
